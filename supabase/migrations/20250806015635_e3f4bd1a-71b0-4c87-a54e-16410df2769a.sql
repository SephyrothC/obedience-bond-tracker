...
SELECT 
  ...
  p.display_name,
  p.role,
  COALESCE(SUM(CASE WHEN pt.type IN ('reward', 'bonus') THEN pt.points ELSE 0 END), 0) as total_points_earned,
  COALESCE(SUM(CASE WHEN pt.type IN ('punishment', 'penalty') THEN pt.points ELSE 0 END), 0) as total_points_spent,
  COALESCE(SUM(CASE WHEN pt.type IN ('reward', 'bonus') THEN pt.points ELSE -pt.points END), 0) as current_points,
